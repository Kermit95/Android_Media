...
public class AudioEncoder {
	...
	private static final String TAG = "AudioEncodedd";
	...
	public AudioEncoder() {
		...
		MediaFormat mediaFormat = MediaFormat.createAudioFormat(
				"audio/mp4a-latm", kSampleRates[1], 2);
		mediaFormat.setInteger(MediaFormat.KEY_AAC_PROFILE,
				MediaCodecInfo.CodecProfileLevel.AACObjectLC);
		mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, kBitRates[1]);
		mediaFormat.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 8192 * 20);
		// It will
		// increase
		// capacity
		// of
	...
	public synchronized void offerEncoder(byte[] input) {
		Log.e(TAG, input.length + " is coming");
		 ...
		 * outData.length); } catch (IOException e) {
		 * catch block e.printStackTrace(); } Log.e(TAG,
